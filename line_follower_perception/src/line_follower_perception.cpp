...
LineFollowerPerceptionNode::LineFollowerPerceptionNode(
  ...
  this->declare_parameter("model_path", "/userdata/resnet18/resnet18_224x224_nv12.bin");
  ...
  point_publisher_ = this->create_publisher<geometry_msgs::msg::Point>(
      "line_point", 10);
  subscriber_ = this->create_subscription<hbm_img_msgs::msg::HbmMsg1080P>(
      "hbmem_img", rclcpp::SensorDataQoS(),
      std::bind(&LineFollowerPerceptionNode::subscription_callback, this,
                std::placeholders::_1));
}
...
bool LineFollowerPerceptionNode::GetParams() {
  ...
  auto parameters = parameters_client->get_parameters({"model_path", "model_name"});
...
int LineFollowerPerceptionNode::PostProcess(
  ...
  line_coordinate_parser->h1_ = h1_;
  line_coordinate_parser->h2_ = h2_;
  ...
  point_msg.point.x = result->x;
  point_msg.point.y = result->y;
  point_msg.point.z = 0.0;

  auto simple_point = geometry_msgs::msg::Point();
  simple_point.x = result->x;
  simple_point.y = result->y;
  simple_point.z = 0.0;

  stamped_publisher_->publish(point_msg);
  point_publisher_->publish(simple_point);
  return 0;
}
...
void LineFollowerPerceptionNode::subscription_callback(
  ...
  ss << "Recved img encoding: "
     ...
     << ", stamp: " << msg->time_stamp.sec << "_" << msg->time_stamp.nanosec
     << ", data size: " << msg->data_size;
  RCLCPP_DEBUG(rclcpp::get_logger("LineFollowerPerceptionNode"), "%s",
               ss.str().c_str());
  ...
  cv::Mat img_mat(msg->height * 3 / 2, msg->width, CV_8UC1,
                  (void *)(msg->data.data()));
  cv::Range rowRange(roi.top, roi.bottom);
  cv::Range colRange(roi.left, roi.right);
  cv::Mat crop_img_mat = hobot_cv::hobotcv_crop(
      img_mat, msg->height, msg->width, 224, 224, rowRange, colRange);

  std::shared_ptr<hobot::dnn_node::NV12PyramidInput> pyramid = nullptr;
  pyramid = hobot::dnn_node::ImageProc::GetNV12PyramidFromNV12Img(
      reinterpret_cast<const char *>(crop_img_mat.data), 224, 224, 224, 224);
  if (!pyramid) {
    RCLCPP_ERROR(rclcpp::get_logger("LineFollowerPerceptionNode"),
                 "Get Nv12 pym fail!");
    return;
...
int LineFollowerPerceptionNode::Predict(
  ...
  RCLCPP_INFO(rclcpp::get_logger("LineFollowerPerceptionNode"),
              "input size:%ld roi size:%ld", dnn_inputs.size(), rois->size());
  return Run(dnn_inputs, output, rois, true);
}
...
int32_t LineCoordinateParser::Parse(
  ...
  RCLCPP_DEBUG(rclcpp::get_logger("LineFollowerPerceptionNode"),
               "PostProcess shape[1]: %d shape[2]: %d shape[3]: %d", shape[1],
               shape[2], shape[3]);
  ...
  const bool normalized_output = std::fabs(x) <= 1.5F && std::fabs(y) <= 1.5F;
  if (normalized_output) {
    result->x = (x + 1.0F) * 960.0F;
    result->y = h1_ + (y + 1.0F) * (h2_ - h1_) * 0.5F;
  } else {
    result->x = x * 2.0F;
    result->y = h1_ + y;
  }

  result->x = std::clamp(result->x, 0.0F, 1919.0F);
  result->y = std::clamp(result->y, 0.0F, 1079.0F);
  RCLCPP_INFO(rclcpp::get_logger("LineFollowerPerceptionNode"),
              "coor rawx: %f, rawy:%f, normalized:%d x:%f y:%f", x, y,
              normalized_output ? 1 : 0, result->x, result->y);
  return 0;
}
...
int main(int argc, char *argv[]) {
  ...
  rclcpp::spin(
      std::make_shared<LineFollowerPerceptionNode>("GetLineCoordinate"));
  rclcpp::shutdown();
  return 0;
}
